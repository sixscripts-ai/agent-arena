...
def _validate_model_ids(databases, database_id: str, user_id: str, model_ids: list[str]) -> None:
    for mid in model_ids:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Unknown model_id: {mid}") from exc
...
def get_battle(battle_id: str, user_id: str = Depends(get_current_user)):
    ...
    return {**{k: v for k, v in battle.data.items() if k not in ("encrypted_key",)}, "id": battle.id}
...
def get_artifacts(battle_id: str, user_id: str = Depends(get_current_user)):
    ...
    return [
        {"phase": d.data["phase"], "model_id": d.data["model_id"], "artifact": d.data["artifact"]}
...
def stream_battle(battle_id: str, user_id: str = Depends(get_current_user)):
    ...
    def event_generator():
        ...
        while True:
            ...
            ordered = sorted(
                events,
                key=lambda e: (e.get("created_at", e.get("ts", 0)), e.get("event_id", "")),
            )
            for ev in ordered:
                eid = ev.get("event_id")
                if eid and eid in seen_ids:
            ...
            if battle.data["status"] in ("completed", "failed", "cancelled"):
                yield {"event": "done", "data": json.dumps({"status": battle.data["status"]})}
                return
