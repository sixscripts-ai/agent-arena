...
type DiffRow = { type: "same" | "add" | "remove"; text: string; oldNo?: number; newNo?: number };
...
function parseExecutionEvent(item: PaneArtifact): EventView {
  ...
  if (raw && typeof raw === "object") {
    const action = String(raw.action || raw.tool || raw.type || raw.event || item.kind || "event").toUpperCase();
    const target = String(raw.target || raw.path || raw.command || raw.file || raw.name || raw.phase || item.phase || "runtime");
    const rawState = raw.status || raw.state || raw.outcome || raw.ok;
    const state = rawState === true ? "ok" : rawState === false ? "failed" : String(rawState || "done");
    const durationValue = raw.duration_ms ?? raw.latency_ms ?? raw.elapsed_ms ?? raw.duration;
    const duration = durationValue === undefined ? "" : typeof durationValue === "number" ? `${Math.round(durationValue)}ms` : String(durationValue);
    const detail = stringifyCompact(raw.result ?? raw.output ?? raw.message ?? raw.detail ?? raw.summary ?? "");
...
export default function CodePane({
  ...
  const versions = useMemo(() => {
    const artifactEvents = history.filter((item) => !item.kind || item.kind === "artifact");
    if (artifactEvents.length) return artifactEvents;
    if (code) return [{ phase: "current", artifact: code, t: 0, kind: "artifact" } satisfies PaneArtifact];
    return [];
  }, [history, code]);

  const activeVersionIndex = selectedVersion !== null && versions[selectedVersion]
    ? selectedVersion
  ...
  const previousArtifact = activeVersionIndex > 0 ? versions[activeVersionIndex - 1]?.artifact || "" : "";
  ...
  const activeFile = selectedFile && files?.[selectedFile] !== undefined ? selectedFile : fileList[0] || null;
  const displayCode = activeFile && files ? files[activeFile] : activeArtifact;
  const lines = displayCode.split("\n");
  const diffRows = useMemo(() => lineDiff(previousArtifact, activeArtifact), [previousArtifact, activeArtifact]);
  ...
  const outputEvents = useMemo(
    () => events.filter((item) => item.kind === "transcript" || item.kind === "action_log").slice(-80),
    [events]
  );
  const latestEvent = events[events.length - 1];
  const isLatest = !versions.length || activeVersionIndex === versions.length - 1;
  ...
  return (
    <section className="card flex h-[560px] min-h-0 flex-col overflow-hidden bg-surface" aria-label={`${label} execution inspector`}>
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex min-w-0 items-center gap-2">
              ...
              {role && <span className="hidden rounded-full border border-border px-2 py-0.5 font-mono text-[8px] uppercase tracking-[0.09em] text-muted sm:inline">{role}</span>}
            </div>
      ...
      <div className="flex min-h-[44px] items-center justify-between gap-2 border-b border-border bg-surface2/35 px-2">
        <div className="flex min-w-0 items-center gap-0.5 overflow-x-auto" role="tablist" aria-label="Inspector views">
          {TABS.map(({ key, label: tabLabel, icon: Icon }) => {
            const active = tab === key;
            const count = key === "versions" ? versions.length : key === "tools" ? toolEvents.length : undefined;
            return (
              <button
                key={key}
                type="button"
              ...
              >
                ...
                {tabLabel}
                {count !== undefined && count > 0 && <span className="text-[8px] opacity-60">{count}</span>}
              </button>
            );
          })}
        </div>
        {!isLatest && versions.length > 0 && (
          <button
            type="button"
            onClick={() => { setSelectedVersion(null); setSelectedFile(null); }}
      ...
      <div className="min-h-0 flex-1 bg-code">
        {tab === "artifact" && (
          <div className="flex h-full min-h-0">
            {files && fileList.length > 0 && (
              <aside className="hidden w-[132px] shrink-0 overflow-auto border-r border-codeBorder bg-code/70 p-2 sm:block" aria-label="Artifact files">
                <div className="mb-2 font-mono text-[8px] uppercase tracking-[0.12em] text-lineNo">work/</div>
            ...
            <div className="flex min-w-0 flex-1">
              <div className="w-11 shrink-0 select-none overflow-hidden border-r border-codeBorder py-3 pr-2 text-right">
                {lines.map((_, i) => (
                  <div key={i} className="font-mono text-[9px] leading-5 text-lineNo">{i + 1}</div>
                ))}
              </div>
              <pre className="h-full min-w-0 flex-1 overflow-auto p-3 font-mono text-[11px] leading-5 text-codeFg whitespace-pre-wrap break-words">
                <code>{displayCode || "// Waiting for the first artifact…"}</code>
              </pre>
            </div>
          </div>
        )}
        ...
        {tab === "diff" && (
          <div className="h-full overflow-auto">
            {versions.length < 2 ? (
              <EmptyState icon={GitCompare} title="No previous version" detail="A diff appears after this agent submits another artifact." />
            ) : (
              <div className="min-w-[560px] py-2 font-mono text-[10px] leading-5">
                <div className="sticky top-0 z-10 flex items-center justify-between border-b border-codeBorder bg-code/95 px-3 py-2 text-[9px] uppercase tracking-[0.08em] text-lineNo backdrop-blur">
                  <span>v{activeVersionIndex || 1} → v{activeVersionIndex + 1}</span>
                  <span>{diffRows.filter((r) => r.type === "add").length} additions / {diffRows.filter((r) => r.type === "remove").length} removals</span>
                </div>
                {diffRows.map((row, index) => (
                  ...
                  >
                    <span className="select-none text-right text-lineNo">{row.oldNo || ""}</span>
                    <span className="select-none text-right text-lineNo">{row.newNo || ""}</span>
                    <span className={`select-none text-center ${row.type === "add" ? "text-emerald-400" : row.type === "remove" ? "text-red-400" : "text-lineNo"}`}>
                      {row.type === "add" ? "+" : row.type === "remove" ? "−" : " "}
                    </span>
                    <span className="whitespace-pre-wrap break-words px-2 text-codeFg">{row.text || " "}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {outputEvents.length ? (
              <div className="space-y-1.5">
                {outputEvents.map((item, index) => (
                  <div key={`${item.t}-${index}`} className="grid grid-cols-[64px_72px_1fr] gap-2 rounded-md border border-codeBorder bg-[#0b0d0f] px-2.5 py-2 font-mono text-[9px] leading-4">
                    <span className="text-lineNo">{timeLabel(item.t)}</span>
                    <span className="uppercase tracking-[0.08em] text-accent">{item.kind || "output"}</span>
                    <span className="whitespace-pre-wrap break-words text-codeFg/85">{item.artifact}</span>
                  </div>
                ))}
              </div>
            ) : (
              <EmptyState icon={Terminal} title="No runtime output yet" detail="Build, test, or transcript output will appear here without growing the page." />
            )}
          </div>
        )}
            ...
            {toolEvents.length ? (
              <div className="space-y-1.5">
                {toolEvents.map((item, index) => {
                  ...
                  return (
                    <div key={`${item.t}-${index}`} className="rounded-md border border-codeBorder bg-[#0b0d0f] px-3 py-2.5">
                      <div className="grid grid-cols-[64px_74px_minmax(0,1fr)_auto] items-center gap-2 font-mono text-[9px]">
                        <span className="text-lineNo">{timeLabel(item.t)}</span>
                        <span className="uppercase tracking-[0.08em] text-accent">{event.action}</span>
                        <span className="truncate text-codeFg">{event.target}</span>
                        <span className={`flex items-center gap-1 uppercase ${failed ? "text-red-400" : "text-emerald-400"}`}>
                          {failed ? <XCircle className="h-3 w-3" /> : <CheckCircle2 className="h-3 w-3" />}
                          {event.state}{event.duration ? ` · ${event.duration}` : ""}
                        </span>
                      </div>
                      {event.detail && <div className="mt-2 border-t border-codeBorder pt-2 font-mono text-[9px] leading-4 text-lineNo break-words">{event.detail.slice(0, 900)}</div>}
                    </div>
                  );
                })}
              </div>
            ) : (
              <EmptyState icon={Wrench} title="No tool activity yet" detail="Structured sandbox actions will appear here when the executor emits action_log events." />
            )}
          </div>
        )}
        ...
        {tab === "versions" && (
          <div className="h-full overflow-auto p-3">
            {versions.length ? (
              <div className="space-y-2">
                {[...versions].reverse().map((item, reverseIndex) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${latest ? "bg-accent" : "bg-lineNo"}`} />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-3">
                          <span className="font-mono text-[9px] font-medium uppercase tracking-[0.08em] text-codeFg">v{index + 1} / {item.phase || "artifact"}</span>
                          <span className="flex items-center gap-1 font-mono text-[8px] text-lineNo"><Clock3 className="h-3 w-3" />{timeLabel(item.t)}</span>
                        </div>
                        <p className="mt-2 line-clamp-2 font-mono text-[9px] leading-4 text-lineNo">
                          {item.artifact.replace(/\s+/g, " ").slice(0, 220) || "empty artifact"}
                        </p>
                      </div>
                    </button>
                  );
              ...
              <EmptyState icon={Layers3} title="No versions yet" detail="Each submitted artifact becomes a selectable version in this fixed panel." />
            )}
          </div>
        )}
      </div>
      ...
      <footer className="flex min-h-[44px] items-center justify-between gap-3 border-t border-border bg-surface px-4 py-2">
        <div className="flex min-w-0 items-center gap-3">
          <span className="truncate font-mono text-[8px] text-muted">{artifactMeta}</span>
          {versions.length > 0 && <span className="hidden font-mono text-[8px] text-muted sm:inline">v{activeVersionIndex + 1}/{versions.length}</span>}
          {toolEvents.length > 0 && <span className="hidden font-mono text-[8px] text-muted md:inline">{toolEvents.length} tool calls</span>}
        </div>
        <span className={`flex shrink-0 items-center gap-1.5 font-mono text-[8px] uppercase tracking-[0.06em] ${win ? "text-warn" : status === "running" ? "text-accent" : "text-muted"}`}>
          {win ? <CheckCircle2 className="h-3.5 w-3.5" /> : status === "running" ? <Radio className="h-3.5 w-3.5" /> : null}
          {win ? winText || "win condition" : latestEvent ? `${latestEvent.kind || "event"} · ${latestEvent.phase}` : status === "running" ? "receiving" : "idle"}
        </span>
      </footer>
    </section>
  );
...
function EmptyState({ icon: Icon, title, detail }: { icon: typeof Braces; title: string; detail: string }) {
  return (
